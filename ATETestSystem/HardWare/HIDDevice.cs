...
namespace ATETestSystem
{
    ...
    public partial class HIDevice
    {
        ...
        [DllImport("hid.dll", SetLastError = true)]
        public static extern bool HidD_GetInputReport(IntPtr DeviceObject, byte[] Buffer, uint BufferLength);

        //HidD_SetFeature 将特征报告发送到指定句柄
        [DllImport("hid.dll", CharSet = CharSet.Auto, SetLastError = true)]
        public static extern bool HidD_SetFeature(IntPtr DeviceObject, byte[] Buffer, uint BufferLength);

        //HidD_GetFeature 返回指定句柄的属性报告。
        [DllImport("hid.dll", SetLastError = true)]
        public static extern bool HidD_GetFeature(IntPtr DeviceObject, byte[] Buffer, uint BufferLength);

        //HidD_GetManufacturerString 返回标识制造商的字符串。
        [DllImport("hid.dll", SetLastError = true, CallingConvention = CallingConvention.StdCall)]
        public static extern bool HidD_GetManufacturerString(IntPtr DeviceObject, IntPtr pointerToBuffer, uint bufferLength);

        //HidD_GetProductString 返回标识制造商产品的字符串。
        [DllImport("hid.dll", SetLastError = true, CallingConvention = CallingConvention.StdCall)]
        public static extern bool HidD_GetProductString(IntPtr hidDeviceObject, IntPtr pointerToBuffer, uint bufferLength);

        //HidD_GetIndexedString 从句柄返回指定的字符串。
        [DllImport("hid.dll", SetLastError = true, CallingConvention = CallingConvention.StdCall)]
        public static extern bool HidD_GetIndexedString(IntPtr hidDeviceObject, uint StringIdex, IntPtr pointerToBuffer, uint bufferLength);
        ...
        public void GetDeviceInfo(int Index, TestInfo StrBuilder)
        {
            try
            {
                if (DeviceOpened)
        ...
        public void GetDeviceList(TestInfo AddItem) // 获取所有连接的hid的设备路径
        {
